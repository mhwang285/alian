...
class EECMC(AnalysisMCBase):
    ...
    def do_eec(self, jet, suffix):
        ...
        for p1, p2 in itertools.permutations(tracks, 2):
            ...
            self.hists[f"eec_T_{suffix}"].Fill(angle, ew, jet.pt(), self.weight)
            if q1 > 0 and q2 > 0:
                self.hists[f"eec_P_{suffix}"].Fill(angle, ew, jet.pt(), self.weight)
            if q1 < 0 and q2 < 0:
                self.hists[f"eec_M_{suffix}"].Fill(angle, ew, jet.pt(), self.weight)
            else:
                self.hists[f"eec_PM_{suffix}"].Fill(angle, ew, jet.pt(), self.weight)
